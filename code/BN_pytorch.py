...
def train_model(files, validation_files, model_out_name, scaler_out_name, n_epochs, train_frac, batch_size, make_epm_output):
    ...
    model = BaselineModel(f"{model_out_name}.pt", bn=True).to(device)



    # how to evaluate the model: process test data, print several metrics
    def evaluate_model(heading):
        model.eval()
        ...
        print(
            f"{heading}: MC loss {test_loss/(batch_idx+1):.5f} | MC AUC: {roc_auc_score(test_tags_np, mypreds):.5f} | MC ACC: {test_acc:.5f}",
            f"| data loss {val_loss/(val_batch_idx+1):.5f} | data AUC: {roc_auc_score(val_test_tags_np, valpreds):.5f} | data ACC: {val_acc:.5f}",
        )
        return mypreds
    evaluate_model("BEFORE adaptation")
    ...
    model.bn2.train()
    model.bn3.train()
    for beg, end in val_train_borders:
        data = val_train_feat[beg:end]
        idx = val_train_idx[beg:end] - val_train_idx[beg]
        output = model(data, idx)
    mypreds = evaluate_model("AFTER adaptation")

    # done training so let's set it to eval
    model.eval()
    torch.save(model.state_dict(), f"{model_out_name}_bn.pt")

    if make_epm_output:
        print("Writing output for EPM")
        try:
            write_epm_file(mypreds, test_tags_np, f"{model_out_name}_bn_epm")
        except ImportError:
